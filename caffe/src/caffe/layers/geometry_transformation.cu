...
__global__ void GeoTransform(const int nthreads, const int H, const int W,
                             const int top_channel, const Dtype *const depths,
                             const Dtype *const transformation,
                             const Dtype *const camIntrinsic,
                             Dtype *const transformed_points) {
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    transformed_points[x_offset] =
        trans_off[0] * X + trans_off[1] * Y + trans_off[2] * d + trans_off[3];
    int y_offset = ((n * top_channel + 1) * H + y) * W + x;
    transformed_points[y_offset] =
        trans_off[4] * X + trans_off[5] * Y + trans_off[6] * d + trans_off[7];
    int z_offset = ((n * top_channel + 2) * H + y) * W + x;
    transformed_points[z_offset] =
        trans_off[8] * X + trans_off[9] * Y + trans_off[10] * d + trans_off[11];
  }
}
...
void GeoTransformLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype> *> &bottom,
  ...
  GeoTransform<<<CAFFE_GET_BLOCKS(n_threads), CAFFE_CUDA_NUM_THREADS>>>(
      n_threads, height, width, top_channel, bottom[0]->gpu_data(),
      bottom[1]->gpu_data(), bottom[2]->gpu_data(), top[0]->mutable_gpu_data());
}

template <typename Dtype>
__global__ void
GetGradient(const int nthreads, const Dtype *const top_diff,
            const Dtype *const depths, const Dtype *const transformation,
            const Dtype *const camIntrinsic, const int height, const int width,
            const int top_channel, Dtype *const depths_diff,
            Dtype *const transformation_diff, Dtype *const camIntrinsic_diff,
            const bool propagate_down_0, const bool propagate_down_1,
            const bool propagate_down_2) {
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    if (propagate_down_0) {
      ...
      float dX_dD =
          trans_off[0] * base_X + trans_off[1] * base_Y + trans_off[2];
      depth_diff_val += top_diff[x_offset] * dX_dD;

      float dY_dD =
          trans_off[4] * base_X + trans_off[5] * base_Y + trans_off[6];
      depth_diff_val += top_diff[y_offset] * dY_dD;

      float dZ_dD =
          trans_off[8] * base_X + trans_off[9] * base_Y + trans_off[10];
    ...
    if (propagate_down_1) {
      // bottom[1] dLdT
      caffe_gpu_atomic_add(top_diff[x_offset] * base_X * depth,
                           transformation_diff + n * 16 + 0);
      caffe_gpu_atomic_add(top_diff[x_offset] * base_Y * depth,
                           transformation_diff + n * 16 + 1);
      caffe_gpu_atomic_add(top_diff[x_offset] * depth,
                           transformation_diff + n * 16 + 2);
      caffe_gpu_atomic_add(top_diff[x_offset] * Dtype(1),
                           transformation_diff + n * 16 + 3);

      caffe_gpu_atomic_add(top_diff[y_offset] * base_X * depth,
                           transformation_diff + n * 16 + 4);
      caffe_gpu_atomic_add(top_diff[y_offset] * base_Y * depth,
                           transformation_diff + n * 16 + 5);
      caffe_gpu_atomic_add(top_diff[y_offset] * depth,
                           transformation_diff + n * 16 + 6);
      caffe_gpu_atomic_add(top_diff[y_offset] * Dtype(1),
                           transformation_diff + n * 16 + 7);

      caffe_gpu_atomic_add(top_diff[z_offset] * base_X * depth,
                           transformation_diff + n * 16 + 8);
      caffe_gpu_atomic_add(top_diff[z_offset] * base_Y * depth,
                           transformation_diff + n * 16 + 9);
      caffe_gpu_atomic_add(top_diff[z_offset] * depth,
                           transformation_diff + n * 16 + 10);
      caffe_gpu_atomic_add(top_diff[z_offset] * Dtype(1),
                           transformation_diff + n * 16 + 11);
    }
...
template <typename Dtype>
void GeoTransformLayer<Dtype>::Backward_gpu(
  ...
  GetGradient<Dtype><<<CAFFE_GET_BLOCKS(n_threads), CAFFE_CUDA_NUM_THREADS>>>(
      n_threads, top_diff, depths, transformation, camIntrinsic, height, width,
      top_channel, depths_diff, transformation_diff, camIntrinsic_diff,
