...
__global__ void InverseWarping(const int nthreads, const int top_channel,
...
template <typename Dtype>
void InverseWarpingLayer<Dtype>::Forward_gpu(
  ...
  InverseWarping<<<CAFFE_GET_BLOCKS(n_threads), CAFFE_CUDA_NUM_THREADS>>>(
      n_threads, top_channel, height, width, bottom[0]->gpu_data(),
      bottom[1]->gpu_data(), top[0]->mutable_gpu_data());
}

template <typename Dtype>
__global__ void
GetGradient(const int nthreads, const int top_channel, const int height,
            const int width, const Dtype *const U, const Dtype *const proj_xy,
            const Dtype *const top_diff, Dtype *const U_diff,
            Dtype *const proj_xy_diff, const bool propagate_down_0,
            const bool propagate_down_1) {
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    for (int cc = 0; cc < top_channel; cc++) {
      ...
      if (x1 >= 0 && x1 <= width - 1 && y1 >= 0 && y1 <= height - 1) {
        // dLdU
        if (propagate_down_0) {
          caffe_gpu_atomic_add(top_diff_this * wx1 * wy1,
                               U_diff + x1 + y1 * width + off);
        }
      ...
      if (x1 >= 0 && x1 <= width - 1 && y2 >= 0 && y2 <= height - 1) {
        // dLdU
        if (propagate_down_0) {
          caffe_gpu_atomic_add(top_diff_this * wx1 * wy2,
                               U_diff + x1 + y2 * width + off);
        }
        // for dLd[x,y] future use
      ...
      if (x2 >= 0 && x2 <= width - 1 && y1 >= 0 && y1 <= height - 1) {
        // dLdU
        if (propagate_down_0) {
          caffe_gpu_atomic_add(top_diff_this * wx2 * wy1,
                               U_diff + x2 + y1 * width + off);
        }
        // for dLd[x,y] future use
      ...
      if (x2 >= 0 && x2 <= width - 1 && y2 >= 0 && y2 <= height - 1) {
        // dLdU
        if (propagate_down_0) {
          caffe_gpu_atomic_add(top_diff_this * wx2 * wy2,
                               U_diff + x2 + y2 * width + off);
        }
        // for dLd[x,y] future use
    ...
    if (propagate_down_1) {
      ...
      proj_xy_diff[proj_x_offset] =
          (topRightProduct - topLeftProduct) * wy1 +
          (bottomRightProduct - bottomLeftProduct) * wy2;
      proj_xy_diff[proj_y_offset] =
          (bottomLeftProduct - topLeftProduct) * wx1 +
          (bottomRightProduct - topRightProduct) * wx2;
...
template <typename Dtype>
void InverseWarpingLayer<Dtype>::Backward_gpu(
  ...
  GetGradient<Dtype><<<CAFFE_GET_BLOCKS(n_threads), CAFFE_CUDA_NUM_THREADS>>>(
      n_threads, top_channel, height, width, U, proj_xy, top_diff, U_diff,
      proj_xy_diff, propagate_down_0, propagate_down_1);
