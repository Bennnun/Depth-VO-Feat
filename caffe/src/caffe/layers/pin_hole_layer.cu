...
__global__ void
PinHoleProjection(const int nthreads, const int H, const int W,
                  const int bottom_channel, const int top_channel,
                  const Dtype *const pts3D, const Dtype *const camIntrinsic,
                  Dtype *const flows, Dtype *const proj_coords) {
...
void PinHoleLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype> *> &bottom,
  ...
  PinHoleProjection<<<CAFFE_GET_BLOCKS(n_threads), CAFFE_CUDA_NUM_THREADS>>>(
      n_threads, height, width, bottom_channel, top_channel,
...
template <typename Dtype>
__global__ void
GetGradient(const int nthreads, const Dtype *const top_flows_diff,
            const Dtype *const top_coords_diff, const Dtype *const pts3D,
            const Dtype *const camIntrinsic, const int height, const int width,
            const int bottom_channel, const int top_channel,
            Dtype *const pts3D_diff, Dtype *const camIntrinsic_diff) {
...
void PinHoleLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype> *> &top,
  ...
  GetGradient<Dtype><<<CAFFE_GET_BLOCKS(n_threads), CAFFE_CUDA_NUM_THREADS>>>(
      n_threads, top_flows_diff, top_coords_diff, pts3D, camIntrinsic, height,
      width, bottom_channel, top_channel, pts3D_diff, camIntrinsic_diff);
