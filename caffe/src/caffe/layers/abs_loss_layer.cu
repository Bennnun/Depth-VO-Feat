...
template <typename Dtype>
__global__ void AbsLossBackward(const int n, const Dtype *diff_data,
                                Dtype *out_diff, const Dtype alpha) {
  CUDA_KERNEL_LOOP(index, n) {
    out_diff[index] =
        alpha * ((diff_data[index] > 0) - (diff_data[index] <= 0));
  }
}
...
void AbsLossLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype> *> &top,
  ...
  for (int i = 0; i < 2; ++i) {
    if (propagate_down[i]) {
      ...
      // ReLUBackward<Dtype><<<CAFFE_GET_BLOCKS(count),
      ...
      /*     caffe_gpu_sign(count, diff_.gpu_data(),
