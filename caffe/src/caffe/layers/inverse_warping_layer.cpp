...
void InverseWarpingLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype> *> &bottom,
  ...
  // CHECK_EQ(3,bottom.size())<< "We need 3 bottoms: Image, horizontal flow and
  // verical flow";
  // CHECK_EQ(1,top.size())<< "Output is only warp image";
  // removed previous check if I want external occusion mask
  // CHECK_EQ(bottom[0]->num(),bottom[1]->num())<< " Input image and the flow
  // should have same number of instances";
  // CHECK_EQ(bottom[0]->height(),bottom[1]->height())<< " Input image and the
  // flow should have same height";
  // CHECK_EQ(bottom[0]->width(),bottom[1]->width())<< " Input image and the
  // flow should have same width";
  // CHECK_EQ(bottom[2]->num(),bottom[1]->num())<< " horizontal and vertical
  // flow should have same number of instances";
  // CHECK_EQ(bottom[2]->height(),bottom[1]->height())<< " horizontal and
  // vertical flow should have same height";
  // CHECK_EQ(bottom[2]->width(),bottom[1]->width())<< " horizontal and vertical
  // flow should have same width";
}
...
template <typename Dtype>
void InverseWarpingLayer<Dtype>::Forward_cpu(
...
template <typename Dtype>
void InverseWarpingLayer<Dtype>::Backward_cpu(
